...
WALLET_METADATA = {
    'name': 'Rainbow',
    'description': '''Rainbow is a non-custodial cryptocurrency browser extension wallet, it supports a number of cryptocurrencies and tokens. 
                    Users are able to easily send and recieve cryptocurrency, as well as interact with decentralised applications.''',
    'websites': ['Rainbow.me'],
    'ext_id': {'chrome':'opfgelmcmbiajamepnmloijbpoleiama'},
    'author': ['CH-CLARK'],
    'plugin-iteration': '1',
...
def rainbow_chrome():
    try:        
        ...
        import_catch = f'ERROR: Rainbow (Chrome) - {e} identified! Execution aborted!'
        pass
        ...
        for x in range(profiles_list_len):
            ...
            if os.path.exists(directory_path + data_location):
                ...
                try:
                    ...
                    for row in csv_data:
                        if row[1] == data_text and 'buster' in row[2] and int(row[3] == accounts_max_seq):
                            most_recent_valuetext = row[2]
                    
                    first_json_obj = json.loads(most_recent_valuetext)
                    json_obj = json.loads(first_json_obj)
                    clientState = json_obj.get("clientState")
                        ...
                        if isinstance(data, list):
                            for items in data:
                                if 'address' in items:
                                    ...
                                    address_output = 'Address', 'Multiple Chain Types', address, wallet_browser, location.replace("\\","/")
