...
WALLET_METADATA = {
    'name': 'Guarda',
    'description': '''Guarda is a non-custodial browser extension wallet, primarily for the Ethereum cryptocurrency and tokens built on the Ethereum network. 
    ...
    'websites': ['guarda.com/chrome-extension/'],
    'ext_id': {'brave':'hpglfhgfnhbgpjdenjgmdgoeiappafln', 'chrome':'hpglfhgfnhbgpjdenjgmdgoeiappafln'},
    'author': ['CH-CLARK'],
    'plugin-iteration': '1',
...
def guarda_brave():
    try:        
        ...
        import_catch = f'ERROR: Guarda (Brave) - {e} identified! Execution aborted!'
        pass
        ...
        for x in range(profiles_list_len):
            ...
            if os.path.exists(directory_path + data_location):
                ...
                try:
                    ...
                    for row in csv_data:
                        if row[1] == data_text and 'defaultWallet' in row[2] and int(row[3] == accounts_max_seq)   :
                            most_recent_valuetext = row[2]

                    json_obj = json.loads(most_recent_valuetext)
                    data_data = json_obj.get('data')
                    wallet_data = data_data['wallets']

                    for x in range(len(wallet_data)):
                        address_output = 'Address', wallet_data[x]['currency'], wallet_data[x]['address'], wallet_browser, location
...
def guarda_chrome():
    try:        
        ...
        import_catch = f'ERROR: Guarda (Chrome) - {e} identified! Execution aborted!'
        pass
