...
WALLET_METADATA = {
    'name': 'Bitcoin Core',
    'description': 'Bitcoin Core is a non-custodial desktop wallet for the Bitcoin cryptocurrency. Users are able to easily send and recieve the Bitcoin cryptocurrency using the platform.',
...
def bitcoin_core():
    try:
        ...
        import_catch = f'ERROR: Bitcoin Core - {e} identified! Execution aborted!'
        pass
    ...
    if operating_system == 'windows':
        ...
        data_location = "/Roaming/Bitcoin/wallets"

    if operating_system == 'mac':
        user_data_subdir = "/Library"
        directory_path = directory_path + user_data_subdir
        data_location = "/Application Support/Bitcoin/wallets"
                    ...
                    with open(wallet_file, 'rb') as file:
                        ...
                        while True:
                            ...
                            if not any(char in stripped for char in special_chars):
                                output_string = 'Address','Bitcoin', stripped, 'Bitcoin Core', directory_path.replace("\\","/") + data_location.replace("\\","/")
