...
WALLET_METADATA = {
    'name': 'Dogecoin Core',
    'description': 'Dogecoin Core is a non-custodial desktop wallet for the Dogecoin cryptocurrency. Users are able to easily send and recieve the Dogecoin cryptocurrency using the platform.',
    'websites': ['dogecoin.com'],
    'ext_id': None,
    'author': ['CH-CLARK'],
    'plugin-iteration': '1',
...
def dogecoin_core():
    try:
        ...
        import_catch = f'ERROR: Dogecoin Core - {e} identified! Execution aborted!'
        pass
    ...
    if operating_system == 'windows':
        ...
        data_location = "/Roaming/Dogecoin"
        
    if operating_system == 'mac':
        user_data_subdir = "/Library"
        directory_path = directory_path + user_data_subdir
        data_location = "/Application Support/Dogecoin"
    ...
    if os.path.exists(directory_path + data_location):
        ...
        if os.path.isdir(directory_path + data_location):
            wallet_file = f'{directory_path + data_location}/wallet.dat'
                ...
                with open(wallet_file, 'rb') as file:
                    ...
                    while True:
                        ...
                        if not any(char in stripped for char in special_chars):
                            output_string = 'Address','Dogecoin', stripped, 'Dogecoin Core', directory_path.replace("\\","/") + data_location.replace("\\","/")
    ...
    if not identified_paths:
        with open(logging_output, 'a') as log_file:
            log_file.write(f'\nERROR: Dogecoin Core - Wallet not found!')
