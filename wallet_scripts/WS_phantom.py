...
def phantom_brave_dump(ask_dir, output_dir):
    ...
    output_path = ask_dir + r"\phantom_brave_LDB.csv"
...
#bfnaelmomeimhlpmgjnjophhpkkoljpa
    if profiles_list:
        for x in range(profiles_list_len):
            profiles_ldb_loc = ask_dir + "/Local/BraveSoftware/Brave-Browser/User Data/" + profiles_list[x] + "/Local Extension Settings/bfnaelmomeimhlpmgjnjophhpkkoljpa"
            profiles = profiles_list[x]
            ...
            if profiles_ldb_loc:
                try:
                    ...
                    for x in range(len(json_obj['value'])):
                        currency_data = list(json_obj["value"].keys())[x]
                        phantom_address_output = 'SOL', currency_data ,'Phantom (Brave)', profiles_ldb_loc
                        
                        phantom_brave_output.append(phantom_address_output) 

                    with open(output_dir + '/' + 'phantom_brave_addresses.csv', 'a', newline='') as file:
                        write = csv.writer(file) 
                        write.writerow(phantom_brave_output)
                    
                    with open(output_dir + '/' + 'WalletSleuth_log.txt', 'a') as log_file:
                        log_file.write('ACTION: (PHANTOM - BRAVE) - Addresses identified in ' + profiles +'.\n')
    ...
    if default_list:
        leveldb_records = ccl_chrome_ldb_scripts.ccl_leveldb.RawLevelDb(ask_dir + r"\Local\BraveSoftware\Brave-Browser\User Data\Default\Local Extension Settings\bfnaelmomeimhlpmgjnjophhpkkoljpa") 
        def_location = ask_dir + "/Local/BraveSoftware/Brave-Browser/User Data/Default/Local Extension Settings/bfnaelmomeimhlpmgjnjophhpkkoljpa"
        ...
        if leveldb_records:
            ...
            for x in range(len(json_obj['value'])):
                currency_data = list(json_obj["value"].keys())[x]
                phantom_address_output = 'SOL', currency_data ,'Phantom (Brave)', def_location
                
                phantom_brave_output.append(phantom_address_output) 

    with open(output_dir + '/' + 'WalletSleuth_log.txt', 'a') as log_file:
        log_file.write('ACTION: (PHANTOM - BRAVE) - Addresses identified in Default Profile.\n')
